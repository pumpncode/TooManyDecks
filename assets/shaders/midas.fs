...
extern Image new_texture;

extern PRECISION float lines_offset;

#define TWO_PI 6.28318530718

vec4 gold_color = vec4(0., 0., 0., 0.);
...
bool line(vec2 uv, float offset, float width) {
    ...
    offset = offset + 0.35 * sin(midas.x + TWO_PI * lines_offset);
    width = width + 0.005 * sin(midas.x);
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    vec4 mpix = Texel(new_texture, texture_coords);
